      *VIEWALLUSERS.cbl
      *Handle paged display of all User records
      *
      *
      *Last updated 03/04/2023 Radio GAGO

       identification division.
       program-id. ViewAllUsers.
           ...
           perform until done
               ...
               perform varying ws-index from ws-first-on-page by 1 until
                 ws-index = ws-first-on-page + ws-records-per-page
                   ...
                   if user-id of file-array(ws-index) is numeric and
                     user-id of file-array(ws-index) > 0
                     and ws-index <= ws-total-records
                       display user-id of file-array(ws-index) at col 2
                         line ws-display-line
                       display user-name of file-array(ws-index) at
               ...
               if ws-go-to-record not equals 0
                   open i-o user-file
                   move ws-go-to-record to user-id of user-RECORD
                   read user-file
                       invalid key
                           close user-file
                           move 0 to ws-go-to-record
                       not invalid key
                           close user-file
                           call "AmendUser" using ws-go-to-record
                           perform BuildFileArray
                           move 0 to ws-go-to-record
               end-if
           end-perform.
           ...
           perform until ws-end-of-file equals 1
               read user-file next record
                   ...
                   not at end
                       ...
                       move user-id of user-record to
                         user-id of file-array(
                           ws-total-records)
                       move user-name of user-record to
                         user-name of file-array(
                           ws-total-records)
